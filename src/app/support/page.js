...
// ISSUE_TYPES lists the categories available when creating a support ticket
const ISSUE_TYPES = [
  "Account Issue",
  "Maintenance Records",
  "Document Reminders",
...
// SupportPage component that lets authenticated users submit support tickets and see submission feedback
export default function SupportPage() {
  ...
  // mark loaded once the session state is no longer resolving
  useEffect(() => {
  ...
  // showToast displays a success or error toast with the given Bootstrap variant and message
  function showToast(variant, message) {
    setToast({ variant, message });
  }

  // handleChange updates the form state when the user changes the issue type or message textarea
  function handleChange(e) {
    const { name, value } = e.target;
    setForm((prev) => ({ ...prev, [name]: value }));
  }

  // submitTicket validates the message and posts the support ticket to the API, showing toast feedback on completion
  ...
  return (
    <>
      ...
      {/* main container for the support page with title, account pill and the ticket form */}
      <div className="rg-support-page min-vh-100 py-4 py-md-5">
        <Container style={{ maxWidth: 1100 }}>
          <div className="d-flex flex-column flex-md-row align-items-md-end justify-content-between gap-3 mb-4">
            ...
            {/* AccountPill showing the signed-in user's email, or a signed-out state if unauthenticated */}
            <AccountPill email={unauthenticated ? null : email} />
          </div>

          <Card className="rg-section-card border-0">
            <Card.Body className="p-4">
              {/* spinner while auth status resolves, login prompt for unauthenticated users, or the ticket form */}
              {(!loaded || status === "loading") ? (
                ...
                <>
                  {/* support banner with contact instructions and a ticket badge */}
                  <div className="rg-support-banner d-flex align-items-center justify-content-between gap-3 p-3 rounded-3 mb-4">
                    ...
                    {/* ticket badge shown in the banner to visually identify this as a support form */}
                    <Badge
                      bg="primary"
                  ...
                  <Form onSubmit={submitTicket}>
                    <Row className="g-3 mb-3">
                      ...
                      <Col xs={12} md={6}>
                        {/* email is read-only so the support team knows who to contact */}
                        <Form.Group>
                          <Form.Label className="text-body-secondary small fw-semibold">
                            EMAIL
                          </Form.Label>
                    ...
                    {/* issue type dropdown — helps categorise the ticket for the support team */}
                    ...
                    <Form.Group className="mb-3" controlId="supportMessage">
                      ...
                      {/* message textarea where the user describes their issue in detail */}
                      <Form.Control
                        as="textarea"
                        name="message"
                        value={form.message}
                      ...
                      />
                      {/* tip prompting users to include steps to reproduce and error messages */}
                    ...
                    <Stack direction="horizontal" gap={2} className="flex-wrap">
                      ...
                      {/* clear button resets the message field without submitting */}
                      <Button
                        type="button"
      ...
      {/* toast container for showing success or error feedback after submitting a ticket */}
      ...
      {/* custom styles for the support page, including the background, form fields and banner */}
      <style>{`
        .rg-support-page {
